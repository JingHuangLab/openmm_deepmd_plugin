...
void testDeepmdDynamicsWithMicromd(vector<VALUETYPE> init_positions, vector<string> particleTypeName, map<string, int> typeDict, vector<double> mass, int nsteps=1000){
    ...
    // For debug.
    cout<<"Num of constraints: "<<context.getSystem().getNumConstraints()<<endl;
    cout<<"Num of forces: "<<context.getSystem().getNumForces()<<endl;
    ...
    for (int ii = 0; ii < nsteps; ++ii){
        ...
        for (int jj = 0; jj < natoms; ++jj){
            ...
            err += diff;
            Vec3 diff_pos = omm_positions[jj] - micromd_positions[jj];
            diff = diff_pos.dot(diff_pos);
            err_pos += diff;
        }
        //ASSERT_EQUAL_TOL(micromd_potential_energy, omm_energy, TOL);
        err_pos = err_pos / natoms;
        err_pos = sqrt(err_pos);
        cout<<"Diff on Positions: "<<err_pos<<endl;
        errorPositions.push_back(err_pos);
