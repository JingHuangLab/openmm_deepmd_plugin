...
def test_deepmd_adaptive_dp_particles_reference(nsteps = 100, time_step = 0.1, Lambda = 1.0, platform_name = "Reference", output_temp_dir = "/tmp/openmm_deepmd_plugin_test_adaptive_dp_particles_output"):
    ...
    crd_file = os.path.join(os.path.dirname(__file__), "../OpenMMDeepmdPlugin/data", "6zfv.crd")
    psf_file = os.path.join(os.path.dirname(__file__), "../OpenMMDeepmdPlugin/data", "6zfv.psf")
    restrain_txt = os.path.join(os.path.dirname(__file__), "../OpenMMDeepmdPlugin/data", "6zfv.restraints_prot_pos")
    sysinfo_file = os.path.join(os.path.dirname(__file__), "../OpenMMDeepmdPlugin/data", "6zfv.sysinfo")
    toppar_file = os.path.join(os.path.dirname(__file__), "../OpenMMDeepmdPlugin/data", "toppar.str")
    
    output_dcd = os.path.join(output_temp_dir, f"6zfv_test.adaptive.dp.region.reference.dp.mm.dcd")
    output_log = os.path.join(output_temp_dir, f"6zfv_test.adaptive.dp.region.reference.dp.mm.log")
    ...
    mini_nstep = 10      # minimization step
    ...
    integrator = mm.LangevinIntegrator(temp*u.kelvin, fric_coeff/u.picosecond, time_step*u.picosecond)
...
def test_deepmd_adaptive_dp_particles_cuda(nsteps = 100, time_step = 0.1, Lambda = 1.0, platform_name = "CUDA", output_temp_dir = "/tmp/openmm_deepmd_plugin_test_adaptive_dp_particles_output"):
    if not os.path.exists(output_temp_dir):
        os.mkdir(output_temp_dir)
    ...
    output_dcd = os.path.join(output_temp_dir, f"6zfv_test.adaptive.dp.region.cuda.dp.mm.dcd")
    output_log = os.path.join(output_temp_dir, f"6zfv_test.adaptive.dp.region.cuda.dp.mm.log")
...
if __name__ == "__main__":
    test_deepmd_adaptive_dp_particles_reference(nsteps = 10, Lambda= 0.5)
    test_deepmd_adaptive_dp_particles_reference(nsteps = 10, Lambda= 0.5)
